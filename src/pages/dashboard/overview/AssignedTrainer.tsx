  ...
  <div className="flex items-center bg-gray-800/40 p-3 rounded-md">
    ...
    <div className="ml-3">
      ...
      <div className="flex items-center mt-1">
        ...
        <span className="text-sm text-teal-300">{trainer.specialization}</span>
      </div>
    </div>
  </div>
);
  ...
  <div className="bg-gray-800/30 rounded-md p-4 text-center">
    ...
    {onRequestTrainer && (
      <button
        ...
        className="mt-3 px-3 py-1 text-xs bg-teal-600 hover:bg-teal-500 text-white rounded-full transition-colors duration-200"
      >
        Request Trainer
      </button>
    )}
...
const AssignedTrainer = ({ trainer, onRequestTrainer }: Props) => (
  <div className="bg-gradient-to-b from-gray-950 to-teal-950/30 rounded-lg shadow-lg p-6 border border-teal-800/30">
    ...
    {trainer ? (
      ...
      <EmptyState onRequestTrainer={onRequestTrainer} />
    )}
  </div>
);
